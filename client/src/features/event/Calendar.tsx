...
export default function Calendar({ currentUser }: { currentUser: any }) {
  ...
  return (
    <div className="min-h-screen bg-[#fffcf5] text-[#521f12] p-6 md:p-14  selection:bg-indigo-100 rounded-[2.5rem] grid grid-cols-1 lg:grid-cols-12 gap-10 ">
      ...
      <div className="lg:col-span-8">
        ...
        <p className="text-gray-400 font-black uppercase text-[10px] tracking-[0.4em]">
         Subdit SIAK
          ...
          <button
            ...
            className="px-5 py-3 bg-amber-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black-600 transition-all"
          >
            + Tambah Agenda
          </button>
        </div>

        <div className="flex items-center gap-3 bg-[#9dac7c] p-1.5 rounded-2xl w-fit mb-4">
          <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1))} className="w-8 h-8 font-bold">←</button>
          <span className="text-sm font-black uppercase min-w-30 text-center">
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
          <div className="grid grid-cols-7 bg-amber-100 border-b border-gray-50">
            {['S', 'S', 'R', 'K', 'J', 'S', 'M'].map((d, i) => (
              <div key={i} className="py-4 text-sm font-black text-gray-400 text-center">{d}</div>
            ))}
          </div>
                  ...
                  {day && (
                    <>
                      ...
                      <div className="flex flex-wrap justify-center gap-1">
                        {dayEvents.map(e => (
                          <div key={e.id} className={`w-1.5 h-1.5 rounded-full ${e.type === 'rapat' ? 'bg-indigo-600' : 'bg-amber-600'}`} />
                        ))}
                      </div>
                    </>
                  )}
